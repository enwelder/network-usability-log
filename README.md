...
| `node tools/radio-join.mjs <session.json> <sysdiagnose.tar.gz>` | attaches the serving cell, the signal, the MIMO layers, the battery temperature and which of Wi-Fi and Bluetooth were live from the phone's own logs to every round, on iOS; [docs/ios-radio-evidence.md](docs/ios-radio-evidence.md) states how to capture one |
