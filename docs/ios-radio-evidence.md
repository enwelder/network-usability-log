...
| a second line can come up by itself | observed | on one ride an eSIM switched off in Settings went from `NotInserted` to live mid-ride with no user action in the log and stayed live; a round records it as `other_sim_lines`, and the chart shades those rounds |
