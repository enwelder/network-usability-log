...
`format: "nulog/session"`, `version: 16`. One button per session writes one JSON file: session
...
| 17 | a literal that failed without stalling is followed by one request to a second literal of its family; the probe carries `via`, `alt_host`, `alt_ms`, `alt_fail`, `primary_ms` and `primary_fail` |
...
| `renamed` | the name was set by hand; the export file is named after it, and after the operator otherwise |
...
| `interrupted` `suspended_ms` | `suspended` when a 250 ms timer fired over 1 s late on either clock, `stop` when Stop ended the round; null otherwise. The row keeps its probes and carries null `grades` and `pgrades`. `suspended_ms` is the largest timer gap over 1 s |
...
| `wake_lock_lost` | the screen lock was released while the round ran and taken again; the round is graded on what it measured. A release between rounds carries no row and is recorded as an event |
...
| `via` `alt_host` `alt_ms` | `ip6` `ip4` | `alt` marks a round trip measured against the second literal of this family, named by `alt_host`, after the configured address failed without stalling. `alt_ms` is that request's time, successful or not |
| `primary_ms` `primary_fail` | `ip6` `ip4` | what the configured address did, kept when the second literal supplied the round trip |
